...
int main() {
  ...
  if (secp256r1_verify_signature(context, test_signature,
    ...
    printf("test fixed data FAILED\n");
  } else {
    printf("test fixed data succeeded\n");
  };

  if (secp256r1_verify_signature(context, lock_bytes, SIGNATURE_SIZE,
                                 &kp.pub_key, message, BLAKE2B_BLOCK_SIZE)) {
    printf("test with public key from private key FAILED\n");
  } else {
    printf("test with public key from private key succeeded\n");
  };
  ...
  if (secp256r1_verify_signature(context, test_signature,
                                 ...
                                 test_message, sizeof(test_message))) {
    printf("test with fixed public_key imported from buffer FAILED\n");
  } else {
    printf("test with fixed public_key imported from buffer succeeded\n");
  };
  ...
  if (secp256r1_verify_signature(context, test_signature,
                                 ...
                                 sizeof(test_message))) {
    printf("test with public_key from arguments FAILED\n");
  } else {
    printf("test with public_key from arguments succeeded\n");
  };
